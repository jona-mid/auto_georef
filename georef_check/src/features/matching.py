...
def load_image(path: str) -> np.ndarray:
    """Load image as RGB numpy array and crop to center 1000x1000."""
    img = Image.open(path)
    if img.mode != "RGB":
        img = img.convert("RGB")

    # Crop to center 1000x1000 (assumes input is 1536x1000)
    w, h = img.size
    if w > 1000 or h > 1000:
        left = (w - 1000) // 2
        top = (h - 1000) // 2
...
def main():
    ...
    # Find test images
    data_dir = Path("georef_check/data/raw/dataset_manual")
